...
function waitForString(getText, label, timeoutSec, needles, isAliveFn) {
    ...
    const check = () => {
      ...
      if (text.length > seenLen) {
        ...
        if (matched.length > 0) {
          ...
          resolve({ matched: true, dead: false, timedOut: false });
          return;
        }
        seenLen = text.length;
      }
      if (typeof isAliveFn === "function" && Date.now() - lastAliveCheck > 500) {
        lastAliveCheck = Date.now();
        if (!isAliveFn()) {
          log(`_wait ${logPrefix(label)} aborting early — process died before needles appeared`);
          resolve({ matched: false, dead: true, timedOut: false });
          return;
        }
      }
      if (Date.now() - start > timeoutSec * 1000) {
        log(`_wait ${logPrefix(label)} timeout after ${timeoutSec}s`);
        resolve({ matched: false, dead: false, timedOut: true });
        return;
      }
