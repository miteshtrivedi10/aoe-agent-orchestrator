...
const DELETION_LOGS_DIR = path.join(path.dirname(REPOS_DIR), "deletion_logs");
...
function checkoutRepo(repoUrl, branch, workDirId) {
  ...
  // Git treats ANY entry in `.git/hooks/` matching a hook name as an
  // executable hook and tries to exec() it. A stray directory (e.g. a
  // force-committed build artifact named `pre-commit`) crashes every commit
  // with "cannot exec: Permission denied". Sanitize after clone so fresh
  // checkouts never inherit broken hooks from the environment.
  sanitizeHooks(ws);

  return { workDir: ws };
}

// Remove non-executable hook entries from a work dir's `.git/hooks/`. Called on
// checkout (lib/repos.js) AND on session start/resume (lib/kilo.js) so buckets
// that were cloned by an older Agent Dock build — or set up outside Agent Dock
// entirely — get their broken directory/socket hooks cleaned before a kilo
// session runs git operations inside them. Git only exec()s regular files and
// symlinks; everything else (directories, FIFOs, sockets) must go.
function sanitizeHooks(workDir) {
  ...
  try {
    for (const name of fs.readdirSync(hooksDir)) {
      ...
      if (!stat.isFile() && !stat.isSymbolicLink()) {
        ...
        log(`sanitizeHooks removed stray hooks entry: ${name} (${workDir})`);
      }
    }
...
// Moves a work directory to a `.trash` sibling immediately, without deleting
// it. The scheduled 03:00 IST cleanup job will hard-remove `.trash` dirs and
// any other repo directories not listed in repos.json. Returns the trash path
// on success; never throws.
function softDeleteWorkDir(workDir) {
  if (!workDir || !fs.existsSync(workDir)) {
    return { renamed: false, trashPath: null, reason: "missing" };
  }
  const parent = path.dirname(workDir);
  const base = path.basename(workDir);
  const trashPath = path.join(parent, `.${base}.${Date.now()}.trash`);
  try {
    fs.renameSync(workDir, trashPath);
    return { renamed: true, trashPath, reason: "rename" };
  } catch (e) {
    log(`softDeleteWorkDir rename failed for ${workDir}: ${e.message}`);
    return { renamed: false, trashPath: null, reason: "rename_failed", error: e.message };
  }
}

// ── Scheduled trash / orphan cleanup ────────────────────────────────────
...
function parseTrashRepoName(name) {
  const m = name.match(/^\.(.+?)\.(\d{13,})\.trash$/);
  return m ? m[1] : null;
}
...
function writeDeletionReport(report, deletionLogsDir = DELETION_LOGS_DIR) {
  try {
    fs.mkdirSync(deletionLogsDir, { recursive: true });
    const file = path.join(deletionLogsDir, `deletion-run-${Date.now()}.log`);
    fs.writeFileSync(file, JSON.stringify(report, null, 2));
    return file;
  } catch (e) {
    log(`writeDeletionReport failed: ${e.message}`);
    return null;
  }
}

// Removes every directory under reposDir that is NOT the current work_dir of a
// bucket in reposFile. This covers `.trash` dirs left by softDeleteWorkDir and
// any orphaned repo directories. Writes a per-run JSON report to
// deletionLogsDir. Defaults point at /data/repos and /data/deletion_logs.
function runTrashCleanup(reposDir = REPOS_DIR, reposFile = REPOS_FILE, deletionLogsDir = DELETION_LOGS_DIR) {
  let repos = [];
  try {
  ...
  const allowed = new Set(
    repos.map((r) => (r.work_dir ? path.basename(r.work_dir) : null)).filter(Boolean)
  );
  ...
  try {
    ...
    log(`runTrashCleanup cannot read repos dir ${reposDir}: ${e.message}`);
    return;
  }
  ...
  for (const ent of entries) {
    ...
    const p = path.join(reposDir, ent.name);
    let files = 0;
    try {
      files = countFiles(p);
    } catch (e) {
      log(`runTrashCleanup countFiles failed for ${p}: ${e.message}`);
    }

    const repoName = parseTrashRepoName(ent.name) || ent.name;
    try {
      child_process.execFileSync("rm", ["-rf", p], { timeout: 600000, stdio: "ignore" });
      deleted.push({ repo_name: repoName, files, deleted_at: toISTString(new Date()), path: p });
      log(`runTrashCleanup removed ${p} (${files} files)`);
    } catch (e) {
      log(`runTrashCleanup failed to remove ${p}: ${e.message}`);
    }
  }

  const report = { run_at: toISTString(new Date()), deleted };
  writeDeletionReport(report, deletionLogsDir);
  log(`runTrashCleanup finished: ${deleted.length} dirs removed`);
}

function next3AM_IST() {
  const now = new Date();
  const fmt = new Intl.DateTimeFormat("en-US", {
    timeZone: "Asia/Kolkata",
    year: "numeric",
  ...
  let next = new Date(`${p.year}-${p.month}-${p.day}T03:00:00+05:30`);
  if (Number.isNaN(next.getTime())) {
    // Fallback if the explicit offset parse fails.
    const offset = 5.5 * 60 * 60 * 1000;
    const istNow = new Date(now.getTime() + offset);
    next = new Date(Date.UTC(istNow.getUTCFullYear(), istNow.getUTCMonth(), istNow.getUTCDate(), 3, 0, 0) - offset);
...
function scheduleTrashCleanup() {
  const run = () => {
    try {
      ...
      log(`scheduled trash cleanup error: ${e.message}`);
    }
  };
  const first = next3AM_IST().getTime() - Date.now();
  log(`scheduled trash cleanup: first run in ${Math.round(first / 1000 / 60)} minutes`);
  setTimeout(() => {
    run();
