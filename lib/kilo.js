...
async function writeProjectConfig(workDir, label, opts = {}) {
  ...
  // Also write the resolved config to <workDir>/kilo.json (at the workdir root,
  // NOT inside .kilo/). The Kilo Cloud Dashboard reads the project config from
  // this path via its FileSystem.readFile RPC — when it's absent the dashboard
  // logs "BadResource: FileSystem.readFile (<workDir>/kilo.json) Failed" and
  // prompt/message submission silently breaks. The CLI still prefers
  // .kilo/kilo.jsonc (higher precedence), so writing both keeps CLI + dashboard
  // happy. Content is identical to the .kilo/kilo.jsonc above.
  const rootConfigPath = path.join(workDir, "kilo.json");
  try {
    fs.writeFileSync(rootConfigPath, JSON.stringify(resolved, null, 2));
    log(`_config_project ${logPrefix(label)} wrote dashboard config to ${rootConfigPath}`);
  } catch (e) {
...
function writeRemoteControlJson() {
  ...
  log(`_startup_config remote_control=true, snapshot=false written to kilo.json (primary) and config.json (legacy)`);
}
