    ...
    if (typeof val === "string") {
      return val.replace(envRe, (match, name) => {
        ...
        if (resolved === undefined) {
          log(`_config_env resolution MISSING for ${name} — leaving placeholder as-is`);
          return match;
        }
        log(`_config_env resolved ${name} (${resolved.length} chars)`);
        return resolved;
      });
    }
...
function writeProjectConfig(workDir, label) {
  let template;
  try {
...
async function startKiloSession(workDir, label) {
  ...
  writeProjectConfig(workDir, label);
...
async function resumeKiloSession(workDir, label, oldCloudSessionId) {
  ...
  if (!tuiReady) {
    log(`_resume_kilo_session ${logPrefix(label)} WARNING — TUI prompt not detected`);
  } else {
  ...
  if (needsNewSession) {
    ...
    const submitPrompt = () => {
      ...
      return sleep(600).then(() => {
        ptyProcess.write("\r");
        log(`_resume_kilo_session ${logPrefix(label)} prompt submitted (text + CR): ${prompt.slice(0, 80)}`);
      });
    };
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      ...
      if (submitted) break;
      log(`_resume_kilo_session ${logPrefix(label)} no session.created after attempt ${attempt} — re-submitting prompt`);
      await submitPrompt();
    }
    if (!submitted) {
      log(`_resume_kilo_session ${logPrefix(label)} WARNING — could not confirm session.created after 3 attempts`);
    }
  ...
  return { pid, ptyProcess, cloudSessionId: resolvedCloudSessionId };
}
...
module.exports = { startKiloSession, resumeKiloSession, initKiloStartup, writeRemoteControlJson, writeDefaultModel, checkGateway, scanInternalLogs, sendPromptToLive, isLive, LIVE_PTYS, writeProjectConfig, resolveEnvVars };