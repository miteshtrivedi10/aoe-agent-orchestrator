...
async function writeProjectConfig(workDir, label) {
  ...
  if (!process.env.KILO_API_KEY) {
    log(`_config_project ${logPrefix(label)} KILO_API_KEY not set — skipping provider block (cloud validation will fail without it)`);
    delete resolved.provider;
  }

  // OpenCode is provided by kilo's native `opencode` provider, which activates
  // automatically from OPENCODE_API_KEY. Custom openai-compatible providers are
  // local-TUI only and are not surfaced in the Kilo Cloud Dashboard, so we rely
  // on the native provider instead of a custom openai-compatible wrapper.
...
async function initKiloStartup() {
  ...
  try {
    ...
    // Work directories are named `<repo>__<sessionId>`, NOT the bare session id.
    // Match against the basename of each tracked session's work_dir so cleanup
    // only removes true orphans and never deletes a live session's clone.
    const activeWorkDirs = new Set(
      kept.filter((s) => s.work_dir).map((s) => path.basename(s.work_dir))
    );
