...
async function resumeKiloSession(workDir, label, oldCloudSessionId) {
  ...
  return { pid, ptyProcess, cloudSessionId: resolvedCloudSessionId, started: submitted || !!resolvedCloudSessionId, reason: (submitted || resolvedCloudSessionId) ? null : (lastReason || "no cloud session id captured") };
}
...
module.exports = { startKiloSession, resumeKiloSession, initKiloStartup, writeRemoteControlJson, writeDefaultModel, checkGateway, scanInternalLogs, sendPromptToLive, isLive, LIVE_PTYS, writeProjectConfig, resolveEnvVars, detectKiloStartupErrors, submitPromptConfirmed };