...
async function initKiloStartup() {
  ...
  try { startLivenessWatchdog(); } catch (e) { log(`_startup watchdog failed (non-fatal): ${e.message}`); }
}
...
// ── Stuck-session liveness watchdog ──────────────────────────────────
// Polls LIVE_PTYS + bucket registry on a fixed interval. Detects a PTY that
// is alive but unresponsive (stuck) and auto-recovers it:
//   1) Graceful: Ctrl-C (abort current turn, non-destructive)
//   2) Escalate: force-kill (SIGTERM→SIGKILL), preserve cloud_session_id
// All thresholds are env-overridable for fast smoke testing.

function startLivenessWatchdog() {
  const INTERVAL = parseInt(process.env.WATCHDOG_INTERVAL_MS || "30000", 10);
  const STARTUP_GRACE_MS = parseInt(process.env.STUCK_STARTUP_GRACE_MS || "120000", 10);
  const SILENCE_MS = parseInt(process.env.STUCK_SILENCE_MS || "300000", 10);
  const GRACE_MS = parseInt(process.env.STUCK_GRACE_MS || "60000", 10);
  const FORCE_KILL_SILENCE_MS = parseInt(process.env.STUCK_FORCE_KILL_SILENCE_MS || "1800000", 10);
  const RELAY_LOST_FORCE_MS = parseInt(process.env.STUCK_RELAY_LOST_FORCE_MS || "120000", 10);

  log(`_watchdog started interval=${INTERVAL}ms silence_ctrl_c=${SILENCE_MS}ms grace=${GRACE_MS}ms force_kill=${FORCE_KILL_SILENCE_MS}ms relay_lost=${RELAY_LOST_FORCE_MS}ms`);

  const timer = setInterval(async () => {
    try {
      const repos = reposLib.loadRepos();
      for (const bucket of repos) {
        try {
          if (bucket.session_state !== "running") continue;
          if (!bucket.pid || !reposLib.isAlive(bucket.pid)) continue;

          const entry = LIVE_PTYS.get(bucket.work_dir_identifier);
          if (!entry) continue;

          const now = Date.now();
          // Guard: do not evaluate until past startup grace AND remote was connected
          if (now < entry.startedAt + STARTUP_GRACE_MS) continue;
          if (!entry.remoteConnectedAt) continue;
          // Skip if already recovered (user must resume)
          if (bucket.recovered) continue;

          const silenceMs = now - entry.lastOutputAt;

          // Stuck signal A: silence > SILENCE_MS → graceful Ctrl-C
          if (!bucket.stuck && silenceMs > SILENCE_MS) {
            log(`_watchdog ${logPrefix(bucket.work_dir_identifier)} STUCK — silence ${Math.round(silenceMs / 1000)}s > ${Math.round(SILENCE_MS / 1000)}s — sending Ctrl-C`);
            try { entry.pty.write("\x03"); } catch (_) {}
            bucket.stuck = true;
            bucket.stuck_since = new Date().toISOString();
            bucket.last_output_at = new Date(entry.lastOutputAt).toISOString();
            reposLib.saveRepos(repos);
            continue;
          }

          // Escalate: force-kill if stuck for > GRACE_MS after Ctrl-C,
          // or if total silence > FORCE_KILL_SILENCE_MS
          if (bucket.stuck) {
            const stuckFor = now - new Date(bucket.stuck_since).getTime();
            if (stuckFor > GRACE_MS || silenceMs > FORCE_KILL_SILENCE_MS) {
              log(`_watchdog ${logPrefix(bucket.work_dir_identifier)} FORCE KILL — stuck for ${Math.round(stuckFor / 1000)}s, silence ${Math.round(silenceMs / 1000)}s`);
              try { await reposLib.terminateProcess(bucket.pid); } catch (_) {}
              bucket.session_state = "stopped";
              bucket.stopped_at = new Date().toISOString();
              bucket.pid = null;
              bucket.is_active_in_agent_dock = false;
              bucket.stuck = false;
              bucket.recovered = true;
              bucket.recovered_at = new Date().toISOString();
              reposLib.saveRepos(repos);
              log(`_watchdog ${logPrefix(bucket.work_dir_identifier)} recovered — cloud_session_id ${bucket.kilo_session_id || "none"} preserved, user can Resume`);
            }
          }
        } catch (innerErr) {
          log(`_watchdog error on bucket ${bucket.work_dir_identifier}: ${innerErr.message}`);
        }
      }
    } catch (err) {
      log(`_watchdog tick error: ${err.message}`);
    }
  }, INTERVAL);
...
module.exports = { startKiloSession, resumeKiloSession, initKiloStartup, writeRemoteControlJson, writeDefaultModel, checkGateway, scanInternalLogs, sendPromptToLive, isLive, LIVE_PTYS, writeProjectConfig, resolveEnvVars, detectKiloStartupErrors, submitPromptConfirmed, checkPtyAlive, consolidateKiloLogs, detectCloudValidationFailure, startLivenessWatchdog };