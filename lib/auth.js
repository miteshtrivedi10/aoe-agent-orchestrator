...
if (TOKEN_IS_AUTOGEN) {
  log(`boot HERMES_API_TOKEN not set — generated ephemeral token (length=${API_TOKEN.length})`);
} else {
  log(`boot HERMES_API_TOKEN loaded from env (${API_TOKEN.length} chars; auto-gen=false)`);
}
if (RATE_LIMIT_DISABLED) log("boot HERMES_RATE_LIMIT=off — rate limits disabled (test mode only)");

function authGate(req, res, next) {
  // Primary: Authorization: Bearer <token>
  // Secondary: X-Hermes-Token: <token> (for use behind HF private-space proxy
  // which consumes the Authorization header for HF token auth)
  const hdr = req.get("authorization");
  const altHdr = req.get("x-hermes-token");
  ...
  if (!rawToken) {
    ...
    return res.status(401).json({
      ...
      hint: "send: Authorization: Bearer <HERMES_API_TOKEN> or X-Hermes-Token: <HERMES_API_TOKEN>",
    });
  }
